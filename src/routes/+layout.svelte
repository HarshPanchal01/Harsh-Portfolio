...
<div class="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)]">
	...
	<header class="fixed top-0 left-0 right-0 z-50 border-b border-[var(--border-color)] bg-[var(--bg-primary)]/95 backdrop-blur-sm">
		<div class="container mx-auto flex items-center justify-between px-4 md:px-6 py-4 max-w-6xl">
			<a href="#top" class="text-lg md:text-xl font-bold text-[var(--text-accent)] hover:text-[var(--accent-cyan)] transition-colors">
				harsh@portfolio:~$
			</a>
			<nav class="flex items-center gap-4 md:gap-6">
				<a href="#experience" class="text-xs md:text-sm hover:text-[var(--text-accent)] transition-colors">experience</a>
				<a href="#projects" class="text-xs md:text-sm hover:text-[var(--text-accent)] transition-colors">projects</a>
				<button
					onclick={toggleTheme}
